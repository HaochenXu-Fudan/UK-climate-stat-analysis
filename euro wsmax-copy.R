...
# out_dir <- "figures_CORDEX_wsmax_byRegion"
...
for (reg in regions) {
  ...
  #用par把五张图画在同一个device里
  #mfrow=c(2, 3)把这个画布分成 6 个小格子
  ...
  for (sea in seasons) {
    ...
    if (is.null(Merged)) {
      plot.new()
      title(main = paste0(reg, " ", sea, " (no obs)"))
      next
    }
    ...
    PlotEnsTS(
      ...
      EnsTransp = 0.15,
      Units = "days",
      main = paste0(reg, " ", sea, " – ", var_name)
    )
  }
  
  ## 最后一个 panel 空出来（2x3=6格，我们只用5格）
  plot.new()
...
#The next step is to move to the state-space smoothing in TimSPEC
#to separate the underlying signal from noise and make the trends easier to interpret.
